...
- `likes` 受 Worker、歌曲数、总 Gate 和同容量 checkpoint 槽位共同限制。每 Lane 的 Governor 直接执行300–399ms字面启动间隔，Worker 不会除掉该值；任务 Gate 额外限制为最多约20次新启动/秒。
...
- song、25评论/页：4与8出口在该输入下都受单一响应+checkpoint链限制，约 `147.01 comments/s`；增加出口不允许同歌并行。
