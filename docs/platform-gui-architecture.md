...
- 平台切换使用“程序化积木交接”：默认是随方向反转的对角积木波，也可选择从中心向边缘扩散的涟漪；业务 DOM 始终不参与动画。
...
- `PlatformWaveTransition.create({ sourcePlatform, targetPlatform, direction, sourceAnchor, targetAnchor, pattern?: "diagonal" | "ripple", commit })` 返回 `{ finished, cancel }`。内部状态固定为 `idle → preparing → covering → covered/commit → revealing → settled`；`commit()` 是唯一允许修改 platform/mode、`hidden`/`inert`、结果/SSE 和 ARIA 的入口，只调用一次并显式返回布尔值。
- 一次过渡约 680 ms。0–326 ms 使用一条连续的覆盖曲线让程序化方块依次放大并闭合间隙，其中 90 ms 只是 pulse 平滑达到全强度的调制点，不是重新起算的阶段边界；约 326 ms/48% 时全视口 alpha 精确为 1，并在源/目标色块完全遮蔽下提交 DOM。326–680 ms 使用一条连续的揭幕曲线按相同次序缩退方块，590 ms 只开始平滑衰减尾部 pulse，同样不发生分支重置。网易云使用石墨/红，QQ 使用深青/薄荷；不使用连续黑幕、紫色渐变、随机粒子、模糊玻璃、泛光云、弹跳或长尾。
- 默认 `diagonal` 以方向相关的左下↔右上顺序和确定性轻微波动传播；`ripple` 以经宽高比修正的视口中心半径和确定性涟漪排序扩散。目标单块尺寸为 34–58 CSS px，网格至少 8×7。方块仅是 Canvas fragment shader 中的视觉隐喻，不读取、截图、分割或移动真实页面。
- 顶栏按钮选择下一次平台切换使用的模式。后端 `/api/preferences` 只接受 `diagonal|ripple`，以锁内原子写保存到稳定运行目录的 `data/ui-preferences.json`，因此桌面端随机 loopback 端口变化后仍可恢复；`ncm-platform-transition-pattern-v1` localStorage 只作为当前 origin 的首屏缓存。两处都不包含身份、任务或结果数据，renderer 写请求按顺序提交，延迟的启动读取不能覆盖更晚的点击。稳定文件缺失/非法时服务端返回 `diagonal`；GET 失败时保留当前 cache/session 值；POST 失败时本次会话与 cache 已生效、稳定文件保持旧值并明确提示保存失败；未知 engine 值回退 `diagonal`。切换进行中修改会明确提示下次生效。reduced-motion、隐藏页面或无 WebGL 时，无论保存哪种模式都立即完成切换。
...
- WebGL2 使用 `gl_VertexID` 全屏三角形：1 program、1 VAO、0 VBO/纹理/FBO/readback，每帧仅一次 `drawArrays(TRIANGLES, 0, 3)`。Fragment shader 通过 `blockGrid`、`blockOrder`、`blockMask` 和 `blockFace` 生成方块，并用 `fwidth` 抗锯齿。使用 `low-power`，关闭 antialias/depth/stencil/preserveDrawingBuffer，开启 premultiplied alpha；DPR 不超过 1.25，颜色缓冲不超过 1,600,000 像素。没有随机数；RAF 中不创建数组/对象/字符串也不查询布局；resize 沿用原 `startedAt`。
- 提交前取消保留源平台，提交后取消保留目标平台；快速 N→Q→N 只收敛到最后选择。应用层在全不透明积木交接内捕获首次提交异常并同栈重试一次；若约 326 ms 的 `commit()` 仍返回 `false` 或抛错，过渡立即结算且不继续揭幕，只有当前应用代际可同步收敛。初始或运行中 reduced-motion、页面隐藏、WebGL2/context/shader/program/VAO/append/resize/draw 失败、首次 RAF 申请失败或 context loss 都立即提交并单次结算 Promise；清理 RAF、resize/visibility/media/context-loss 监听、VAO、program、context、Canvas、`platform-switching` 和 `aria-busy`。`pagehide` 清理，`pageshow` 若 desired platform 与当前不同则无动画收敛，并通过提交返回值保证当前 generation 的 SSE 只连接一次。取消平台或其他界面动画时，正在过渡的 `<details>` 依 `data-expanded` 同步 `open`/`aria-expanded` 并清理动画状态。
...
- 积木 Canvas 在动画结束后一帧内从 DOM 移除，不留 `requestAnimationFrame`、事件监听器或显存资源；测试同时证明两种模式、90/590 ms 调制点连续、提交点全屏 alpha=1、每帧仅一次全屏三角形 draw、无随机数/额外 GPU 资源/业务 DOM 动画写入。
