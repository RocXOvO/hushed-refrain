...
左侧 rail 的主列表只保留任务配置，footer 保留全局设置；实时结果、活动歌曲、运行日志、出口详情和速度估算由主区 tabs 与 Inspector 提供，不再复制导航入口。`<=820px` 时任务配置与全局设置分居紧凑 rail 两端，两个按钮都保持固定宽度，不横向拉伸或制造空白列。

UID 帮助只引导用户在浏览器打开网易云音乐网页版、进入目标用户主页，并读取地址栏 `id=` 后连续的数字；不再建议通过客户端或分享入口获取 UID。
...
启动时更新检查保持静默，之后以 30 分钟为周期单飞轮询。页面隐藏或 pagehide 时不保留轮询定时器，恢复可见或从 BFCache 返回时按上次完成时间补齐，到期则立即静默检查。只有确认存在更高版本后，右上角更新按钮才出现，后台检查不得自动打开更新弹窗。用户点击按钮时复用已确认的通用 Release 或 Windows 原生 updater 快照；已知新版的下载失败状态仍保留入口供重试。
...
- `PointerSilkTrail.create({ host, platform, enabled })` 仅在 `#mainWorkspace` 内惰性创建一张 `pointer-events:none` 的 WebGL2 Canvas，返回 `setEnabled` / `setPlatform` / `suspend(reason)` / `resume(reason)` / `destroy`。它只接受 `pointer:fine` 的 mouse 移动，不捕获指针、不阻断业务交互，也不覆盖导航、顶栏、右侧 Inspector 或弹窗。4 条独立绢线各持有 20 个控制点，确定性参数以递增 spring `[0.041,0.054,0.068,0.079]` 反向配对递减 friction `[0.898,0.867,0.834,0.802]`，避免 high-spring/high-momentum 共振；每帧先从 point 19 倒序传播到 point 1，使 follower 读取前一帧 predecessor，再更新 head，避免正序更新造成同帧塌缩。head 相对其目标偏移位置的 lag 还经过 `MAX_HEAD_LAG_PX=32` 软限幅，限幅后同步回写速度。速度仍以 0.15 平滑；world-space 线宽和偏移经相机平面投影为 CSS 像素，线宽上限 22 px、偏移半径上限 26 px，两端在 10% 范围内收尖。
- 启动权只属于真实的 fine-mouse `pointermove`：首次合法坐标以及 mouse `pointerenter` 只建立坐标基线并标记几何待刷新，不创建 Canvas、seed 或 RAF；此后的 client 坐标变化或非零 `movementX/Y` 才允许 setup/schedule。`wheel` 与捕获阶段 `scroll` 会立即清空画面并取消 RAF，滚动只把 geometry 标为 dirty，不读取布局或改写 backing buffer；下一次真实移动才单次刷新几何并启动全新尾迹。`resume(reason)` 只解除对应暂停原因，不调度或恢复旧轨迹。
- 热路径仅复用预分配的 `Float32Array`：80 个控制点、4 组速度/偏移和 160 个宽线顶点；每帧一次 `bufferSubData` 上传后执行 4 次 `TRIANGLE_STRIP` draw，材质 opacity 为 0.76，无纹理、FBO、随机数、粒子、模糊、后处理、业务 DOM 读取或每帧对象分配。客户端不引入 Vue/Three.js 运行时，只使用一个低功耗 WebGL2 program/VAO/VBO。片元着色器以 clamped `fwidth` 分别稳定侧缘与端帽的 framebuffer-pixel 覆盖，并 clamp 最终颜色；首点 incoming 退化时使用 forward 方向，避免退化切线形成尖角。最后一次移动后保持 72 ms，再用 348 ms 淡出，到 420 ms 立即清空并将空闲 RAF 降为 0。渲染 DPR 不超过 1.25，backing buffer 每边向上取整且不得低于 1 backing px/CSS px；2,000,000 像素预算只削减超采样，实际像素上限为 host CSS 面积与约 2,000,000 的较大者。Canvas 与 ResizeObserver 只在首次合格移动后分配，上下文/编译/上传/绘制/RAF 异常会锁存失败并完整释放 GPU 表面。
...
- 状态反馈使用实色黑曜表面、14 px 中等圆角、1 px 低对比边框、克制阴影、18 px 状态图标、单行标题与弱化说明；入场只做 8 px 上移与轻微缩放，不从极小圆点夸张膨胀。普通提示、启动进度和池状态共享相同表面语言，但保留各自的语义图标、细进度与可忽略的“详情”动作。
...
- 用户来源的 `sourceNotices` 与 `sourceErrors` 必须分开：未公开听歌排行/喜欢的音乐显示为友好的中性来源提示并注明已跳过，不向任务区输出原始 422/英文调用栈；其他来源继续运行，单一私密来源以确认 0 首正常结算但覆盖不完整。喜欢歌单返回显式、非空、合法且唯一的可访问 `trackIds`，但数量少于列表/详情声明时，也属于中性提示：立即扫描可访问子集、显示缺少数量并保持覆盖不完整，不重试同一确定性差异或轮转出口。缺失目录、正数声明配空向量、非法/重复 ID、owner 或普通自建歌单完整性错误仍保留错误语义；旧检查点中的隐私 422 字符串在展示层向前兼容映射为同一提示。
- 用户来源目录读取期间，任务区必须显示当前阶段（听歌排行、喜欢歌曲、自建歌单或准备扫描）和已经实际启动的目录请求数，不能在远端工作期间长期表现为无说明的 `running / 0`。目录阶段是最佳努力的实时反馈，不参与预算或完成判定；第一条评论页请求开始后清除阶段提示。
...
- 活动表只展示未完成工作和达到页数上限的警告行；正常完成（包括“已完成可读范围”）的歌曲立即移出 DOM、计数和行缓存，不在“正在扫描的歌曲”中长期占位。任务总体完成数仍由任务指标与检查点负责。
- 主工作区使用视口剩余高度而不是固定内容高度：实时结果、并行歌曲、运行日志和出口详情面板在高纵向分辨率下伸展到主内容底部，长数据仍在各自表格内滚动。空表只在其可用区域内垂直居中；普通或矮窗口保留 220 px 表格下限并允许文档自然滚动，不能为了填满大屏反向压缩任务信息或制造横向溢出。
...
- 鼠标尾迹测试必须证明 Canvas 惰性且只属于 `#mainWorkspace`，输入只有 fine mouse，首次合法坐标与 `pointerenter` 只建基线，只有后续真实位移才能创建表面和启动 RAF；冷态与热态的 `wheel`/捕获阶段 `scroll` 都不能创建或重启尾迹，滚动突发不读取布局或更新 backing，同坐标且零 movement 的合成移动必须被忽略，之后的真实移动只刷新一次 geometry 并可启动新尾迹，`resume` 也不得自行调度。4×20 链按 point 19→1 倒序传播后才更新 head，递增 spring 与递减 friction 使用确定性抗共振配对，并保留 32 px head-lag 软限幅。对 100 px 半径、持续 240 帧的圆周输入，必须分别扫描 0.18、0.20、0.215、0.265 rad/frame，并验证四条线的全部 20 个点始终处于 160 px 包络内。world-space 宽度/偏移投影后分别受 22 px/26 px 上限约束，峰值 opacity 为 0.76，每帧只有一次上传和 4 次 40 顶点 `TRIANGLE_STRIP` draw。实现还必须断言首点 forward fallback、颜色 clamp、侧缘/端帽的 clamped `fwidth` AA，backing buffer 每边 `ceil` 且不低于 host CSS 尺寸，DPR≤1.25，约 2,000,000 像素只限制超采样；实现不得引入纹理、FBO、随机数、粒子、模糊、后处理或业务 DOM 捕获。420 ms 空闲、弹窗、平台切换、hidden/blur 后不得留 RAF 或过期轨迹，关闭、减少动效、粗指针与 pagehide 还必须释放 Canvas/GPU 资源，BFCache 只重建一个实例。设置测试需覆盖 v1→v2迁移、默认 true、partial update、桌面原子持久化与浏览器会话隔离。
- 状态胶囊测试必须覆盖共同 DOM 容器、任务启动/代理池/Toast 同时出现时的垂直堆叠、Inspector 折叠/展开、starting/refreshing/stable、点击展开，以及相同轮询状态不重复写 DOM；真实 CSS reload 后还要在桌面、`1280/821` 与 `820/390` 断点测得栈到右栏为正间距、窄屏展开 Inspector 时不覆盖且文档无横向溢出。高纵向分辨率 QA 还要验证四个表格型面板填满主区剩余高度、空状态居中、长表格内部滚动，并在 900×640 与 390×844 下没有被最小高度截断。PDF 文件名测试必须覆盖双平台完整目标、Windows 禁止/控制字符、尾部点号/空格、设备保留名、180 字符主文件名上限和固定 `.pdf`，PDF smoke 必须覆盖 renderer-to-IPC 链路、累计耗时单调性和完整阶段序列。桌面关闭测试必须覆盖系统关闭与自绘按钮、取消、后台、记住选择、重复/过期决定、renderer 不可用，以及退出期间的最终检查点状态；真实 Electron QA 还要确认没有原生 `showMessageBox`、关闭弹窗与客户端视觉一致。
...
- 来源选择测试必须覆盖滑动高亮的几何对齐、听歌排行↔喜欢歌曲快速反向、排行范围的 `aria-hidden`/`inert` 与 reduced-motion 静态收敛；还要覆盖串行/代理池的排行与喜欢隐私、单一/组合来源、覆盖不完整、旧 422 文案映射，以及真实故障仍为错误。有效的部分喜欢目录必须只请求一次且不换 Lane，保留可访问歌曲、缺失提示和不完整覆盖；缺失/空但声明为正数/非法/重复目录继续拒绝。目录阶段测试还要证明可见文案和物理请求数前进，第一条评论请求会清除阶段。响应式真实 QA 在 Windows `win32` 1293×841 CSS px（对应 1940×1261@150% 截图）确认顶栏和文档无溢出、badge 单行、任务面板间隔 21 px；并在 1481/1480/1381/1380/1294/1293/1281/1280/1238/1237/1211/1210/1000/900/821/820/390 px 全部验证无 topbar/document 溢出。网易云↔QQ 在 1381/1293/1280/1238/1237/900 px 的 portal 锚点均为 `dx=0`、`dw=0`，browser 820/390 px 也无横向溢出。
...
- 两平台的真实发车都先取 Gate 容量和至少 50 ms 聚合启动槽，再在 HTTP 边界预约 Lane Governor；物理重试不绕过。20 starts/s 是理论硬上限，不是实网观测。四种 GUI 扫描视图以及 NetEase source/parallel 的新 Server/CLI 任务都默认每 Lane 为 300 ms + `U[0,100)` ms，即 300–399 ms；历史恢复值与用户显式配置不被新默认覆盖。NetEase source 仍默认 1 Worker/出口；8 出口、32 Worker 需 `workersPerProxy=4`、`hostConcurrency=32`。本机一次 synthetic 离线集成基准在 32 独立 parent、8×4 Worker、300+`U[0,100)` ms、50 ms Gate、200 ms 合成 RTT、满 40 回复页下，1280 条/1795 ms，约 713 replies/s；不承诺实网性能。
