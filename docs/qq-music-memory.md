...
- 歌曲 ID 与 SeqNo 始终是十进制字符串。`requestedSongId` 是 song 任务主键，元数据响应不得替换它或经过 JavaScript `Number`。
...
- 耐久顺序是结果 `write + fsync` → 再检查取消 → 提交 counters/seen key/cursor → 等待 checkpoint revision。JSONL 可领先状态，cursor 不可领先 JSONL；恢复必须对账而不重复追加/计数。
- `song` 每页 checkpoint；`likes` 以 400 ms 或 4 个脏页先到者 flush，并用动态 Gate 容量限制未持久页。所有终态等待已开始的 flush 并强制保存。

## Worker、代理与取消

- 每 Lane 拥有独立 Client/Governor，任务共享一个 QQ TransportGate。Governor 的 `minDelayMs + jitter` 是字面请求启动间隔，Worker 永不除小它。
- 新任务每 Lane 默认 300–399 ms；任务 Gate 的聚合启动间隔至少 50 ms。
...
- 当前 web cache-busters 为 styles 59、platform-wave 15、pointer-silk-trail 1、app 70；GUI/WebGL/Canvas2D 细节只维护在 `platform-gui-architecture.md`。
