...
- 生产扫描 parser 与单次解码实验隔离：正式 start 的数字 QQ 或 URL 数字通过独立 4 秒本机直连辅助 Lane 解析为 canonical `EncryptUin`；直接 opaque EncryptUin 或 URL 中的 opaque EncryptUin 原样作为 canonical 值并保持本地零请求，不要求可被实验字符表反解。安全与实验细节见“EncryptUin / 官方主页单次解析实验”；实验结果绝不替换扫描 task key、generation、状态/结果路径或作者匹配条件。
...
- 歌曲搜索优先使用 `music.search.SearchCgiService.DoSearchForQQMusicDesktop`。只有主响应结构完整且 `body.song.list` 合法为空时，才用同一 `fetch` 和取消 signal 回退到官方 `smartbox_new.fcg`；主列表非空但任意记录畸形时必须报协议错误，不能用回退掩盖。Dashboard 的歌名/数字歌曲查询强制使用一条 4 秒有界的本机直连 Lane，不读代理池或表单手动代理；前端同关键词单飞、缓存 60 秒且显示加载/失败状态。Smartbox 只取 `limit` 内的 `data.song.itemlist`，ID/MID 保持字符串，`singer` 规范化为单元素 artists 数组。该回退依据一次低频实网兼容性观察，常规验证仍只使用 stub。
...
- loopback-only `POST /api/qq/encrypt-uin/verify` 由 `QQJobManager.verifyClassicEncryptUin` 通过一条 4 秒有界的本机直连 Lane，以原 canonical EncryptUin 和解码候选发出两次公开资料请求，比较 EncryptUin、昵称和头像；只返回 `{format,identityKind,status,maskedIdentifier,checks}`。解析/验证不创建或改变扫描 generation。格式失败、网络/限流失败、上游不可验证和 mismatch 必须分开呈现；match 不证明所有权或私密数据访问权。
- 可信本地界面按展示规则允许出现完整 QQ 或 opaque EncryptUin；日志、错误、诊断、导出文件名、真实文档示例和 Release 说明仍不得泄漏这些值。直接 19 位数字仍完整显示为 QQ；只有 28 字符 Token 解码出的内部 ID 不在界面显示。测试只用合成向量，必须覆盖完整 QQ/opaque、“微信用户”、三个 URL 正例与全部 URL 拒绝边界、本地零网络、数字/验证固定直连且绕过池与手动代理、4 秒上限、超时/取消/lease 释放、match/mismatch、普通目标后台资料补全不阻塞 scanner、微信用户零补全请求/固定展示，以及 generation/checkpoint 不变。
...
- 两平台搜索统一返回 `{ platform, query, songs:[{ id, mid?, name, artists, album?, durationMs? }] }`。网易云 `/api/song/search` 使用 `cloudsearch` 单曲类型，并与纯数字 `/api/song` 一样使用 8 秒有界本机直连；QQ 歌曲查询使用 4 秒上限。启动前 `/api/user` 固定直连，以 100ms Governor 间隔并行读取资料、排行和喜欢来源，单请求 2 秒上限，前端同 UID 单飞并缓存 60 秒；明确缺少目标自有喜欢歌单时返回 `private` 并显示“已开启隐私”，不与冷却或畸形数据混淆。普通查询都不读代理池或手动代理。搜索的前端请求世代、旧响应丢弃、Canvas 切换与选中态由 GUI 层负责，后端不把搜索绑定到扫描 generation。NetEase Live Task `/api/user/profile` 仍必须携带 `mode + jobId` 并在请求前后复核 UID/generation，但资料请求本身固定本机直连，不复用 source/parallel Lane、Governor 或 Gate。
...
- 每个平台任务提交共用右下角点扩胶囊：宽度约292px、内容列占满可用空间，只显示按时序切换的启动阶段、细进度条与短暂成功/失败结算，不显示耗时，也不替代正式任务进度或检查点。启动成功/失败不再重复弹 Toast；若任务极快结束，结算弹窗必须等胶囊离场后再留 240ms 间隔，避免连续通知突跳。
...
- 当前缓存版本是 `styles.css?v=58`、`platform-wave.js?v=15`、`app.js?v=68`；修改资源后只递增对应 token，并与 `web/index.html` 同步。
