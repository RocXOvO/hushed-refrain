...
- `web/platform-wave.js` 用一次性 WebGL2 程序化方块在约 680 ms 内完成平台交接。默认 `diagonal` 依方向形成左下↔右上的积木波，顶栏可选经宽高比修正、从中心向边缘扩散的 `ripple`；偏好经严格 `/api/preferences` 写入稳定运行目录的 `data/ui-preferences.json`，localStorage 仅是随机 loopback origin 的首屏缓存，切换中修改明确从下次生效。0–326 ms 是连续覆盖曲线（90 ms 仅是 pulse 调制点），约 326 ms/48% 的全视口 alpha=1 帧唯一提交目标工作区，326–680 ms 是连续揭幕曲线（590 ms 仅开始尾部平滑衰减），不会在两个调制点重置或闪回。目标块为 34–58 CSS px、至少 8×7；渲染器只使用 `gl_VertexID` 全屏三角形、一个 program/VAO、每帧一次 `TRIANGLES` draw，不创建 VBO、纹理、FBO 或 readback。业务 DOM 不写 `transform`/`opacity`/`filter`/`will-change`，共享外壳几何与滚动位置不抖动；目标结果异步回填后会按当前平台代际二次恢复非零滚动坐标，避免空表钳制为 0。切换期间新触发的 WAAPI、弹窗和 Toast 无动画即时呈现，Canvas 是唯一继续变化的元素。提交前取消保留源平台，提交后取消保留目标平台；两种模式在 reduced-motion、隐藏页、WebGL 缺失/抛错、初始化/绘制失败或 context loss 时都即时安全结算并释放 RAF、监听器、VAO、program、context 与 Canvas。
...
- 当前缓存版本是 `styles.css?v=50`、`platform-wave.js?v=6`、`app.js?v=62`；修改资源后只递增对应 token，并与 `web/index.html` 同步。
...
运行 `npm run check`、`npm test`、`npm run build`、`npm run bench:qq`、`node --check web/app.js`、`node --check web/platform-wave.js`、`npm run desktop:smoke:mac` 与 `git diff --check`。浏览器需验收双平台/四 viewKey、mode 目标快照立即切换、重选当前平台、QQ lookup 的 busy 仅由异步 `finally` 释放、平台/模式/启动竞态隔离、旧请求/SSE 抑制、`committed=false` 上层恢复、两种积木模式及偏好刷新、稳定文件非法默认、local cache/API 失败的分层保留语义、提交前/后取消、`low-power` 与显式 context 释放、各异常降级、两种模式下的运行中 reduced-motion、双平台 rail/导航项等宽、横纵双轴缩放、overlay 面板互斥及内部滚动、矮屏 EncryptUin 弹窗、1281→1280 与 821→820 断点、所有桌面平台 900 px 登录按钮可访问名称、无横向溢出和 0 console error。通过本地门禁不等于已 commit、push、打包或发布；Release 仍需独立核对版本、tag、提交和平台资产。
