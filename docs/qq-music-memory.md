...
- 两平台搜索统一返回 `{ platform, query, songs:[{ id, mid?, name, artists, album?, durationMs? }] }`。网易云 `/api/song/search` 使用 `cloudsearch` 单曲类型，并与纯数字 `/api/song` 共用 Router：显式代理优先，运行中的已验证池轮换起点并对瞬态失败最多尝试三个出口，复核/请求失败都不回退直连，未运行池时使用直连。搜索的前端请求世代、旧响应丢弃、Canvas 切换与选中态由 GUI 层负责，后端不把搜索绑定到扫描 generation。NetEase Live Task 的 `/api/user/profile` 则必须携带 `mode + jobId`，前后复核 UID/generation，并复用对应活动 source/parallel 任务的 Lane、Governor 与 Gate；任务通道关闭或失败时明确报错，绝不另建直连。
