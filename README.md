...
| 设置 | 可选值 | 作用 |
| --- | --- | --- |
| 主机并发上限 | `1..32`，默认 `8` | 限制整个任务在所有出口上的同时请求总数，用户来源与单曲并行模式共用该上限 |
| 任务出口上限 | `0..32`，默认 `0` | 限制本次任务最多使用多少个已验证出口；`0` 表示轮换使用全部已验证出口 |

请在启动任务前修改；任务运行或停止过程中输入框会锁定，避免中途改变调度拓扑。顶部“拓扑”文字只是当前设置的实时摘要，例如“自动使用 8/8 出口 · 主机≤8并发 · 每出口≤3线程”，不是第三个设置入口。如果客户端中没有“主机并发上限”输入框，请升级到 `v0.13.0` 或更高版本并完全重启客户端。

自动模式会把全部已验证出口加入本次任务，再由主机级并发门控制同一时刻最多多少个请求。即使任务使用 8 个出口且每出口配置 3 个 Worker，主机上限设为 8 时也仍然最多只有 8 个代理请求同时进行；多出的 Worker 会排队，并让后续工作能够轮换到其他出口。只有手动填写正数“任务出口上限”时才会缩小本次任务的出口集合。

每个代理出口是一个 Lane，同一 Lane 上的所有 Worker 共享一个 `RequestGovernor`。Worker 按出口轮询启动，避免靠前出口先把队列拿光；当歌曲数少于可用主机并发时，用户来源任务会在启动阶段就把未读时间范围预分成互不重叠的工作项，把空闲 Worker 分配到同一歌曲的不同时间段，而不是等待第一轮歌曲完成后再扩容。恢复旧任务时，如果断点里只有少量未完成分片，也会在首轮请求前按新的可用容量继续安全拆分，不必先串行扫完一页。

主机并发是传输上限，不会凭空创建 Worker。实际配置 Worker 数是“任务实际出口 × 每 IP 并发”，还受代理池可用出口数和手动出口上限约束。例如只有 4 个任务出口且每 IP 为 3 时，即使主机上限填 18，也只有 12 个 Worker；界面会直接提示拓扑未占满上限。并行歌曲页分别显示“不同歌曲数”和“活跃 Worker / 配置 Worker”：歌曲会按 ID 去重，多 Worker 扫同一首仍只算一首，活跃数也会随请求间隔和响应速度波动。同一歌曲有多个请求并行时，“正在请求第 N 页”取仍在运行的最小页码，已完成页数与请求中页码分开统计。
