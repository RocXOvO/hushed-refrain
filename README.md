...
早期版本会让 Worker 一直扫完一首歌或一个时间分片。评论密度不均时，后期只剩少数“大任务”，其他 Worker 会提前退出，看起来像请求被风控。当前版本采用**一页一个工作项 + 启动预分片 + 运行中自适应拆分**：处理完一页后重新入队；歌曲少于可用 Worker 时会先把未读范围拆到主机容量，运行中再继续按空闲 Worker 拆分。因此只有一首候选歌时，也能让多个代理出口并行扫描互不重叠的时间范围。若启用了每歌页数上限，最后一个允许页自然返回结束会按完整覆盖结算；只有达到上限后仍有未读范围才标记为截断。
...
采用目标 UID owner 校验后，“喜欢歌曲 / 两者”使用带 `target-v2` 后缀的新检查点和结果文件。这是一次有意的安全隔离：新版“喜欢歌曲 / 两者”任务不会继续读取或追加旧版未校验 owner 的歌曲目录和命中结果。听歌排行任务仍使用原检查点及历史共享结果路径，可照常续跑并继续展示该路径中已有的去重结果。
