...
普通启动会刷新所选目录，再按歌曲 ID 与检查点对账：已完成歌曲复用覆盖，未完成歌曲从原 cursor/分片继续，新增歌曲从头扫描。组合来源的单项失败会保留其他成功来源并标记覆盖不完整，不会破坏旧断点。并行歌曲的完成度按持久化时间覆盖判断，不拿会变化的评论总数做终态依据；自然完成的歌曲会在任务仍运行时保留为 100% 完成状态。若配置了每首最大评论页数，达到上限且仍有未读范围时会明确显示“达到页数上限、未覆盖全部评论”，不会伪装为完成；恰在最后允许页自然结束仍按完整完成结算。
