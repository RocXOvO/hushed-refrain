...
- **实时可观测**：右侧展示同时扫描的歌曲、名称和 Worker 数，主区只保留总进度、累计请求、命中与实时评论。
...
每个代理出口是一个 Lane，同一 Lane 上的所有 Worker 共享一个 `RequestGovernor`。任务栏的“任务出口”可以手动限定本次扫描使用的独立出口数；设为 `0` 时按主机总并发上限和每 IP Worker 数自动选择。例如主机上限为 8、每 IP 3 Worker 时会从已验证代理池中选择 3 个出口，实际请求仍受主机级 8 并发保护。
...
桌面界面可以自动发现 Clash Verge 的配置和 Mihomo 内核，默认从 48 个候选节点中优选并保留 8 个出口。代理池容量与单次任务出口数相互独立：任务可只使用其中一部分，且不会重建或缩小共享代理池。可以勾选任意一套，也可同时勾选多套；合并时会在不同配置之间公平轮转、去除重复节点并安全改名重名节点。也可切换到“其他代理池”，每行导入一个 HTTP/HTTPS 代理。
