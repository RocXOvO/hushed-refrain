...
- **实时可观测**：中央“并行歌曲”大页面展示正在扫描的歌曲、名称和 Worker 数；主区持续显示总进度、累计请求、命中与实时评论。
...
| 设置 | 可选值 | 作用 |
| --- | --- | --- |
| 主机总并发 | `1..32`，默认 `8` | 限制整个任务在所有出口上的同时请求总数，用户来源与单曲并行模式共用该上限 |
| 任务出口上限 | `0..32`，默认 `0` | 限制本次任务最多使用多少个已验证出口；`0` 表示根据主机总并发自动计算 |

请在启动任务前修改；任务运行或停止过程中输入框会锁定，避免中途改变调度拓扑。顶部“拓扑”文字只是当前设置的实时摘要，例如“自动使用 3/8 出口 · 主机≤8并发 · 每出口≤3线程”，不是第三个设置入口。如果客户端中没有“主机总并发”输入框，请升级到 `v0.13.0` 或更高版本并完全重启客户端。

自动出口数按 `ceil(主机总并发 / 每出口 Worker)` 计算，并受当前代理池可用出口数限制：

| 主机总并发 | 每出口 Worker | 自动使用出口 |
| ---: | ---: | ---: |
| 4 | 3 | 2 |
| 8 | 3 | 3 |
| 12 | 3 | 4 |

即使手动把“任务出口上限”设为 `8`，主机总并发设为 `8` 时也仍然最多只有 8 个代理请求同时进行；增加出口主要是分散单个公网 IP 的负载和提高故障切换余量，不会绕过主机总并发上限。

每个代理出口是一个 Lane，同一 Lane 上的所有 Worker 共享一个 `RequestGovernor`。任务栏可以把“主机总并发”设为 `1..32`；“任务出口上限”可手动限定本次扫描使用的独立出口数，设为 `0` 时按 `ceil(主机总并发 / 每 IP Worker)` 自动计算。例如主机上限为 8、每 IP 3 Worker 时会从已验证代理池中选择 3 个出口，实际请求仍受主机级 8 并发保护。

少用出口的意义是减少本地连接、监听端口和节点维护开销，同时仍能填满设定的主机并发；多用出口则能分散单个公网 IP 的请求量，但在主机总并发不变时不会让总吞吐自动增加。界面会明确显示“已用/可用出口、主机并发上限、每出口 Worker”，避免把代理池容量误认为当前任务并发。
...
| `src/proxy-lane-selection.ts` | 按手动上限或主机并发容量选择本次任务实际出口 |
...
项目不使用数据库。评论结果是追加写入的 JSONL，扫描状态是 JSON，实时 UI 失败不得中断结果持久化。客户端启动扫描时会流式读取历史 JSONL 并定期让出事件循环，避免大结果文件造成窗口卡顿。实时结果按搜索发现顺序展示（最新发现优先），不会再按评论发布时间重排；喜欢列表只返回 ID 时会在后端批量补全歌曲名称并写入检查点。底层依赖固定为 `@neteasecloudmusicapienhanced/api@4.39.0`，避免上游变更直接影响正在续跑的任务。
