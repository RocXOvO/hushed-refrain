...
两个网易云视图的“读取楼中楼回复”默认开启。关闭后只扫顶层评论，不会发出楼中楼请求，并使用独立的 `-root-only` 状态、结果和覆盖路径；两种范围不共用已完成记录、覆盖或结果。

普通启动会刷新所选目录，再按歌曲 ID 与当前评论范围的检查点对账：已完成歌曲复用覆盖，未完成歌曲从原顶层 cursor/分片和可选楼中楼游标继续，新增歌曲从头扫描。组合来源的单项失败会保留其他成功来源并标记覆盖不完整，不会破坏旧断点。
...
`v1.2.0` 的交付基线为 589/589 项测试（0 fail / 0 cancelled），并通过类型检查、构建、QQ benchmark、三个 renderer 语法检查、macOS 桌面冒烟和 diff-check。真实浏览器验收确认两个楼中楼开关默认开启、可反复切换且提示正确，console warning/error 为零。Windows 安装器由 GitHub Actions 在与 Release 相同的精确提交上执行完整测试、打包应用启动/PDF 冒烟与 NSIS 资产验证；自动化测试默认使用内存桩和回环代理，不会向音乐平台发起真实批量扫描。
