...
- **完整 PDF 导出**：从已落盘结果生成 A4 报告，显示阶段、累计耗时、取消和明确失败信息。
- **克制的桌面交互**：平台切换使用一次性 WebGL2 绢缎折幕；主工作区提供可关闭的四线 MeshLine 鼠标尾迹，4×20 个控制点保留多帧长度，抗共振参数与 32 px 头部滞后软限幅使不同速度的持续画圈仍保持有界，投影线宽最高 22 px，静止 420 ms 后停止渲染。
...
普通启动会刷新所选目录，再按歌曲 ID 与检查点对账：已完成歌曲复用覆盖，未完成歌曲从原 cursor/分片继续，新增歌曲从头扫描。组合来源的单项失败会保留其他成功来源并标记覆盖不完整，不会破坏旧断点。
...
- 右侧 Inspector 收起时，构建或后台复测会显示可点击的全局进度提示。
...
`v1.1.5` 的完整测试基线为 535/535。Windows 安装器由 GitHub Actions 在真实 Windows 环境中执行完整测试、打包应用启动/PDF 冒烟与 NSIS 资产验证；自动化测试默认使用内存桩和回环代理，不会向音乐平台发起真实批量扫描。
