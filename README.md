...
续跑会沿用同一任务结果文件并按评论 ID 去重，所以报告展示的是该 UID/来源结果文件截至导出时的累计完整结果，而不是仅当前这次启动新增的条目。评论、昵称和歌曲名只作为文本输出，不会作为 HTML 执行；超长评论会无损拆成带“续”标记的打印行跨 A4 页面排版，避免 Chromium 在一个超长表格行跨页时发生文字重叠。
