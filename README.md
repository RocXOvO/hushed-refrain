...
普通启动会刷新所选目录，再按歌曲 ID 与检查点对账：已完成歌曲复用覆盖，未完成歌曲从原顶层 cursor/分片和楼中楼游标继续，新增歌曲从头扫描。组合来源的单项失败会保留其他成功来源并标记覆盖不完整，不会破坏旧断点。

网易云的 `comment_new` 页只返回顶层评论，但上游总数包含顶层与楼中楼回复。客户端因此显示“已搜索顶层 + 回复 / 上游总数”，并分别列出顶层页与楼中楼页；总数尚未可用时只显示搜索数和页数，不伪造百分比。底层仍以顶层时间覆盖与每个楼中楼的 `hasMore=false` 共同判定完成，不拿 `totalCount` 或 `replyCount` 当终态依据。如果已完成当前可读取范围但计数仍低于上游提示，界面会如实标记，不强制显示 100%。可信发行时间只用于顶层覆盖核算，实际顶层扫描仍保守覆盖到 2000 年。

“每首最大页数”只限制顶层 `comment_new` 页；楼中楼按总请求上限继续。达到顶层上限且仍有未读时会明确显示“达到页数上限、未覆盖全部评论”；恰在最后允许页自然结束时，仍要等已发现的楼中楼全部结束才完整结算。请求上限按逻辑历史/顶层/楼中楼页计数；同一顶层页跨出口故障转移会复用已保留的请求额度和顶层页上限名额，物理重试、目录请求和歌曲信息补全也不重复扣减。这一口径同样适用于串行 CLI 的历史、顶层和楼中楼页。
...
桌面数据保存在 Electron `userData`；源码运行使用项目内 `data/` 与 `.ncm/`。项目不使用数据库：扫描状态是 JSON，结果和日志是追加式 JSONL。网易云结果 writer 使用长连接私有权限文件，完整写入并同步每条命中后才发布结果和推进检查点；旧文件末尾若缺换行，会先持久补齐分隔，而不覆写原始片段。这些运行目录均被 Git 忽略。
...
PDF 从当前 generation 对应的已落盘结果快照生成；网易云楼中楼命中会标明“楼中楼回复”及父评论 ID。单曲并行报告在顶层时间覆盖不到 100% 时只说任务尚未完整；只有顶层已到 100% 但整体未完成时才明确标注“楼中楼尚未完成”，而顶层和楼中楼都完成才写完成。Windows 默认文件名显示完整 UID/EncryptUin，但会清理禁止字符、尾部点号/空格和设备保留名；日志、错误与诊断继续脱敏。单份报告上限为 64 MiB JSONL 或 20,000 条命中，超过时会明确拒绝而不是静默截断。
...
`v1.1.8` 的已发布完整测试基线为 549/549。当前 `v1.1.9` 候选已通过 `npm run check`、576/576 项测试（0 fail / 0 cancelled）、build、QQ benchmark、三个前端语法检查、macOS 桌面冒烟和 diff-check。较早的本地浏览器验收已覆盖默认尺寸、900×640 与 390×844 的顶层页上限/楼中楼完成文案，无横向溢出或 console warning/error。该候选尚未发布，也不代表 Windows 包已通过 workflow。Windows 安装器由 GitHub Actions 在真实 Windows 环境中执行完整测试、打包应用启动/PDF 冒烟与 NSIS 资产验证；自动化测试默认使用内存桩和回环代理，不会向音乐平台发起真实批量扫描。
