...
- **克制的桌面交互**：平台切换使用一次性 WebGL2 绢缎折幕；主工作区提供可关闭的四线 MeshLine 鼠标尾迹，静止 420 ms 后停止渲染。
...
`v1.1.3` 的完整测试基线为 529/529。Windows 安装器由 GitHub Actions 在真实 Windows 环境中执行完整测试、打包应用启动/PDF 冒烟与 NSIS 资产验证；自动化测试默认使用内存桩和回环代理，不会向音乐平台发起真实批量扫描。
