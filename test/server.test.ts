...
test("dashboard serves UI assets and estimate API", async (context) => {
  ...
  assert.match(pageText, /id="statusCapsuleStack"[^>]*class="status-capsule-stack"[\s\S]*id="poolBuildNotice"[\s\S]*id="taskStartupProgress"[\s\S]*id="toast"/);
  ...
  assert.match(styleText, /@media \(max-width:\s*820px\)[\s\S]*body\.inspector-collapsed \.status-capsule-stack\s*\{[^}]*right:\s*82px[^}]*left:\s*12px[\s\S]*body:not\(\.inspector-collapsed\) \.status-capsule-stack\s*\{[^}]*visibility:\s*hidden/s);
  assert.match(styleText, /\.pool-build-notice\s*\{[^}]*position:\s*relative[^}]*border-radius:\s*14px/s);
  ...
  assert.match(styleText, /\.task-startup-progress\s*\{[^}]*position:\s*relative[^}]*border-radius:\s*14px/s);
