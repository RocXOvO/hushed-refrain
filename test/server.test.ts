...
test("dashboard serves UI assets and estimate API", async (context) => {
  ...
  assert.match(platformWaveText, /const GRID_SPACING_CSS_PX = 30/);
  assert.match(platformWaveText, /const GRID_COLUMNS_MIN = 19/);
  assert.match(platformWaveText, /const GRID_COLUMNS_MAX = 58/);
  assert.match(platformWaveText, /const GRID_ROWS_MIN = 14/);
  assert.match(platformWaveText, /const GRID_ROWS_MAX = 36/);
  assert.match(platformWaveText, /const pointCount = gridColumns \* gridRows/);
  assert.match(platformWaveText, /float crest = exp\(-pow\(distanceToCrest/);
  assert.doesNotMatch(platformWaveText, /Math\.random|TRIANGLE_STRIP|translate3d|will-change/);
