...
class FakeUpdater extends EventEmitter implements WindowsUpdaterBackend {
  ...
  async checkForUpdates(): Promise<void> {
    ...
    this.emit("update-available", {
      version: "0.4.0",
      releaseName: "Hushed Refrain v0.4.0",
  ...
  async downloadUpdate(): Promise<void> {
    ...
    this.emit("update-downloaded", { version: "0.4.0", releaseName: "Hushed Refrain v0.4.0" });
  }
  ...
  updater.checkForUpdates = async () => {
    updater.emit("update-available", {
      ...
      releaseNotes: `<h2>更新内容</h2><ul>
        ...
        <li>并发 &amp; 估算同步 &#x1F680;</li>
      </ul><p>感谢使用&nbsp;Hushed Refrain。</p><script>alert("hidden")</script>`,
    });
  };
