...
- Current web cache-busters are `styles.css?v=65`, `platform-wave.js?v=15`, `pointer-silk-trail.js?v=6`, and `app.js?v=79`; keep them synchronized with `web/index.html`.
...
- NetEase `CommentScope` is `root-only-v1 | root-and-floor-v1`; both dashboard scan views default to the latter. `root-only-v1` performs zero floor discovery/request I/O and uses separate `-root-only` state, result, and coverage paths. Scope is an immutable compatibility key: completion, coverage, checkpoints, and results are never reused across scopes.
...
- The `v1.2.0` delivery baseline is 589/589 tests with zero failures/cancellations, plus check, build, QQ benchmark, three renderer syntax checks, macOS desktop smoke, and diff-check. Browser QA verified both NetEase floor switches default on, repeated toggling and scope-specific hints with console warning/error at zero. The exact release commit must additionally pass the Windows workflow/package gate before its five assets are published.
