...
1. User-source scan (`scan`, `/api/job`, `runPooledCommentFinder`)
   ...
   - With a verified pool, one lane is created per task-selected distinct egress, not necessarily every entry kept in the shared pool. A song page is initially one queue item; after a successful page, unfinished songs are requeued so Workers fairly rotate instead of one Worker monopolizing a long song.
...
Source Workers have stable invocation-local IDs (`lane-name:worker-number`). Their page request `start`/`success`/`failure` activity carries the Worker ID plus song ID/name; `JobManager` tracks activity by Worker and publishes `activeSongs`, grouped by distinct song with an active Worker count and kept in first-active insertion order. Terminal/error settlement clears the activity map. The legacy-compatible `currentSong`/`onSongProgress` snapshot still carries aggregate page progress, but the workspace renders the concurrent `activeSongs` list instead of presenting one song as the whole task.
...
- Source live activity comes from request activity keyed by Worker. `activeSongs` describes only requests currently in flight and groups multiple Workers on the same song; it is not a completion list, queue snapshot, or promise that a missing song is finished. `currentSong` remains an aggregate compatibility field from `onSongProgress`, not the runtime pane's data source. Page numbering shown to users is one-based.
...
- The workspace has one task-total progress bar. Source global progress is completed songs over selected songs; parallel global progress is cursor-weighted time coverage. The runtime pane separately lists active songs and Worker counts without inventing a per-song completion percentage. `totalComments` remains optional scanner/report data, and `mergeCommentTotal` keeps the maximum of stored total, latest credible total, and processed count; UI percentages never determine task completion.
...
- Repeated identical job and active-song snapshots do not rewrite their DOM. SSE matches are buffered into short render batches and the visible-result map remains bounded; `visibleResultOrder` keeps API/SSE discovery order with newest discoveries first, so rendering/pruning does not repeatedly sort every row by comment timestamp. A high match rate must not rebuild the whole results table once per comment. The log tab refreshes at most about every three seconds and skips identical table payloads.
...
- Managed-pool capacity and per-task exit selection are different settings. Pool size controls how many distinct verified exits remain online. GUI field/server input `maxProxyLanes` accepts `0..32`: `0` is automatic and targets `ceil(8 / workersPerLane)` exits, while a positive value is a manual upper bound. `selectProxyLanes` takes the ordered prefix and caps either mode at verified availability; its `mode`, `available`, `requested`, and actual `selected` metadata travel in the job snapshot/log. The form, toolbar, estimator, source/parallel start, and version-1 resume descriptor must use the same rule. Static-proxy/direct source scans still have exactly one lane and no pool selection metadata. Never rebuild or shrink the shared pool to implement this per-task subset, never claim requested lanes as actual lanes, and never fall back from a failed expected pool to direct traffic.
...
- Every `EnhancedNcmClient` request carries a 30-second upstream timeout by default. Network/`5xx`/408/425 failures first receive bounded Governor retry; a final ordinary lane failure is requeued through `LaneRecovery` with exponential 1..30 second backoff and a later success resets that lane. Required work uses `RequestGovernor.execute`: `403`/`429` latch a shared terminal cooldown, wake waiters, and block that lane for the invocation so unfinished comment work remains checkpointed. Optional enrichment uses `executeBestEffort` (and `executeBestEffortProxyRequest` when transport-gated): it keeps the same request budget, start spacing, retries, shared transport gate, and cancellation, but its own `403`/`429` is thrown only to the optional caller and must not poison later required comment requests. Never use best-effort policy for source discovery, comment pages, or other correctness-required traffic. Operator stop still wakes Governor delay/backoff waiters and cancels queued transport work before another remote start.
...
- `AsyncWorkQueue` is the completion detector for pooled scanners: `take()` waits when the visible queue is empty but work remains in flight; every taken item must call `complete()` exactly once; `stop()` wakes waiters and rejects further requeue. Queue removal uses an amortized O(1) head cursor and compacts only after at least 1,024 consumed slots and at least half the backing array; preserve FIFO across compaction and requeue. Do not restore repeated `Array.shift()`, which is O(n) and previously amplified long-run renderer/process load.
...
| `src/api.ts` | Adapter over pinned `@neteasecloudmusicapienhanced/api`; 30-second default timeout, response/error normalization, and de-duplicated multi-ID song detail calls. |
...
| `src/proxy-transport-gate.ts` | Task-wide proxy first-hop concurrency/start smoothing plus required and best-effort Governor wrappers. |
...
| `web/app.js` | UI/drawer state, callback-fed job polling, ordered/batched SSE results, task-lane estimates, and native update UX. |
| `web/styles.css` | 16 px-root responsive workspace/drawer styling, integrated thin transparent-track scrollbars, and reduced-motion handling. |
...
- The dashboard shows one global progress bar: source counts completed songs and parallel uses cursor-weighted time coverage. The right runtime pane lists the distinct songs currently in flight with Worker counts; it is activity, not completion authority. Comment totals remain optional API data, and the runtime timer freezes at the server's terminal elapsed value.
...
For topology/metadata/performance changes, focused evidence must cover the pure auto/manual lane selector; indexed 500-ID metadata batching, partial first-batch retention plus durable checkpointing after a later failure, old-checkpoint hydration, and serial fallback; required `403/429` stickiness versus best-effort metadata `403` and `429` followed by successful required work; best-effort retry/budget/cancellation parity; and a pooled session where a cooled metadata lane is attempted once, skipped in later batches, healthy lanes continue, and the main scan remains usable. Also preserve source Worker activity, source/parallel checkpoint callback counters, and FIFO behavior through queue compaction/requeue. Server asset regex tests prove wiring only. Visually exercise the actual drawer and inspector at desktop width and at/below 1120 px, verify the task drawer starts closed, every main output-tab path closes it, focus leaves hidden drawer content, and keyboard/ARIA/reduced-motion behavior remains valid; a stylesheet substring is not responsive or animation QA. Result-order tests must use comments whose historical timestamps disagree with arrival order so an accidental timestamp sort is detectable.
...
- Changing the managed pool's build size is not the same as choosing a smaller lane subset for one scan. Do not rebuild or destructively shrink the shared pool merely to honor `maxProxyLanes`, and do not change `0` from its automatic host-capacity meaning to "all exits".
...
The `codex/ui-v0-replit-redesign` branch is the user-requested visual implementation and deliberately remains separate from the legacy `main` UI until review. Preserve these characteristics when iterating on that branch; do not merge it into the production branch without explicit review.

- Treat the product as one operational surface, not five independent pages. The desktop implementation uses a permanent 58 px icon-only rail and an initially closed, absolute task drawer (up to 336 px) so configuration does not reserve a second wide column. Search/settings open the drawer; starting a task and every main output-tab path (`results`, `logs`, `pool`, `estimate`, including already-active/programmatic paths) close it. Opening from the task-bar button activates Search, while closing a Search/Settings drawer restores the active output-tab item, so the rail never highlights hidden content. If focus was inside the drawer when it closes, move focus to the task-panel open button with `preventScroll`. Preserve button titles/labels, `aria-current`, `aria-expanded`/`aria-hidden`, focusability, and visible active state.
- A top task bar concentrates UID, source mode, actual worker/exit counts, and start/stop actions. The central work area and the collapsible right runtime/node-detail pane remain the information hierarchy; collapsed controls retain tooltips, accessible names, focusability, and a visible active state.
- The center should emphasize overall progress, scanned count, and matches; the right runtime pane owns the distinct songs currently in flight and their Worker counts. Live results and logs use switchable compact tables/panels.
...
- Visual direction: neutral surfaces, one accent color, thin borders, moderate radii, clear typography with a 16 px root size, small state labels, and collapsed advanced settings by default.
- The task drawer uses a 210 ms horizontal transform plus short opacity transition; mode/disclosure changes use similarly short user-triggered motion based primarily on compositor-friendly `transform`/`opacity`. Avoid animating dense live lists, polling updates, or long-running decoration. Honor `prefers-reduced-motion`, keep final hidden/ARIA/focus state correct, and do not let an interrupted animation leave controls inaccessible.
- At widths `<=1120px`, boot collapses the right inspector and CSS turns it into an overlay/collapsed edge control so the main work area keeps its width; at `<=820px`, navigation becomes a compact top rail and both side surfaces overlay the main content. Responsive behavior must not depend on a stale renderer stylesheet/script: bump the query-string cache-buster in `web/index.html` whenever `web/styles.css` or `web/app.js` changes.
- Avoid purple gradients, heavy glass effects, pervasive rounded cards, long blur work, or layout-thrashing animations. Preserve integrated scrollbars, single-flight polling, snapshot signature checks, bounded result DOM, batched SSE rendering, and log throttling. Starting a task, switching panels, and receiving live results must remain responsive while persistence/network work runs asynchronously.
- Renderer refactors should give each behavior one owner: task submission/availability, polling snapshots, result buffering, navigation/panel state, and formatting must not each grow parallel source/parallel copies. Extract pure helpers or focused modules when useful, but preserve the server API, checkpoint schemas, CSP/static-asset rules, and distinct scan-engine semantics. Remove redundancy only with focused behavioral tests; visual similarity is not proof that source and parallel lifecycles can be merged.
