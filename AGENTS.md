...
- Current release line: `v1.1.6`. `package.json` and root `package-lock.json` are the version authorities; GitHub Release/tag/assets must still be checked separately when publication state matters.
...
- Current web cache-busters are `styles.css?v=62`, `platform-wave.js?v=15`, `pointer-silk-trail.js?v=6`, and `app.js?v=74`; keep them synchronized with `web/index.html`.
...
- The `v1.1.6` release baseline is 536/536 tests with check, build, benchmark, all three renderer syntax checks, macOS desktop smoke, and diff-check passing; real browser QA covered the source-selector highlight, reversible scope collapse, rapid reversal, and layout stability at 1280×720, 900×640, and 390×844 with zero console errors. GitHub publication still requires the exact commit/tag/assets checks below.
