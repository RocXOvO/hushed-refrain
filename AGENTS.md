...
- Per-song completion remains owned by durable half-open time coverage, never by the upstream live `totalComments` count. Each song persists its immutable `commentEndTime`; trusted release metadata may set `coverageStartTime` for backend coverage accounting while the Scanner still reaches the global 2000 safety bound. The client deliberately presents `commentsProcessed / totalComments` as the visible progress ratio; if the upstream total is unavailable it shows the searched count and pages without inventing a percentage. A naturally completed row stays visible as `已完成`; a configured per-song page cap is a distinct `truncated` terminal state and must say `达到页数上限`, with its count ratio capped below a false 100% completion.
