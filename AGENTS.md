...
- Current package candidate: `v1.1.9`; the published tag/Release remains `v1.1.8` until the final commit is pushed and publication is verified. `package.json` and root `package-lock.json` are the version authorities; GitHub Release/tag/assets must still be checked separately.
...
- Current web cache-busters are `styles.css?v=65`, `platform-wave.js?v=15`, `pointer-silk-trail.js?v=6`, and `app.js?v=78`; keep them synchronized with `web/index.html`.
...
- Formal NetEase comment scope is `root-and-floor-v1`. `comment_new` returns top-level comments only, even though its `totalCount` includes top-level comments plus floor replies. Page it with a strictly descending time cursor, default page size 1000, accepted range 1..2000; a `hasMore` page must advance and an empty advancing page is valid. A root's normalized `replyCount` is only a floor-discovery hint, never completion authority.
- Each discovered `(songId,parentCommentId)` floor is an independent durable thread. Page `comment_floor` in batches of 40, starting at `time=-1`; while `hasMore=true`, the next time cursor must be an integer that strictly increases. Only `hasMore=false` completes a floor; `replyCount` and floor `totalCount` may be stale after moderation. On resume, drain persisted unfinished floors globally before scheduling more root pages. Matching floor rows must finish the existing JSONL write before that thread's cursor, counters, and checkpoint advance.
...
- Per-song completion requires both durable half-open root-time coverage and every persisted floor thread at `hasMore=false`; `rootDone` alone is not song or coverage completion. Each song persists its immutable `commentEndTime`; trusted release metadata may set `coverageStartTime` for root coverage accounting while the Scanner still reaches the global 2000 safety bound. The client presents searched top-level comments plus replies over the upstream combined total, and lists top-level pages separately from floor pages; totals are display hints, not terminal authority. If a completed readable range remains below a stale combined total, say `已完成当前可读取范围` rather than forcing a false 100%.
...
- Split `[startTime,endTime)` into non-overlapping half-open root-time shards and paginate each with descending `comment_new` cursors, without a login cookie. Persisted floor threads keep independent ascending cursors and are drained before resumed root work; all root shards and all floor threads must finish before the scan is complete.
...
- `JsonlResultWriter` owns one long-lived append handle opened with mode `0600`, loops until each complete record is written, and `sync()`s every record before de-dup ownership, `onAppend`, cursor/count advancement, or checkpoint publication. Initialization preserves a malformed trailing fragment for diagnosis but durably inserts a newline before any future valid row; repair write/sync failure immediately closes that handle. Normal write/sync failure is latched for all later appends. `close()` rejects new appends but drains every append it already accepted before closing, and every serial, pooled, parallel, stop, failure, and cancellation exit awaits it.
- NetEase source state is version 4 with `commentPagination:"cursor-v1"` and `commentScope:"root-and-floor-v1"`; source state and coverage paths are target-v4, while the de-duplicated canonical result JSONL deliberately remains target-v3. State owns per-song root cursors/shards, independent `(song,parent)` floor cursors/counts, source catalog version 3, cooldown and truncation. Pre-v4 scan checkpoints cannot prove floor coverage and are re-enumerated under v4 while reusing the target-v3 result set; v4 coverage is isolated from the old ledger.
...
- Parallel state is version 2 and binds immutable range/configured-shard/page-size identity plus persistent floor threads. Adaptive child root shards and ascending floor cursors are both persisted and resumed.
...
- The published `v1.1.8` release baseline is 549/549 tests. The current `v1.1.9` candidate has passed `npm run check`, 576/576 tests with zero failures/cancellations, build, QQ benchmark, all three renderer syntax checks, macOS desktop smoke, and diff-check. Earlier browser QA at the default viewport, 900×640, and 390×844 verified the top-level page-cap/floor-completion copy with no horizontal overflow or console warning/error. This candidate is not yet a published release or a verified Windows package; GitHub publication still requires the exact commit/tag/assets checks below.
