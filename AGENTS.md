...
- Current release line: `v1.1.5`. `package.json` and root `package-lock.json` are the version authorities; GitHub Release/tag/assets must still be checked separately when publication state matters.
...
- Current web cache-busters are `styles.css?v=61`, `platform-wave.js?v=15`, `pointer-silk-trail.js?v=6`, and `app.js?v=72`; keep them synchronized with `web/index.html`.
...
- The `v1.1.5` release baseline is 535/535 tests with check, build, benchmark, all three renderer syntax checks, macOS desktop smoke, and diff-check passing; real browser QA compared the bounded Follow trail on both platforms and under sustained circles. GitHub publication still requires the exact commit/tag/assets checks below.
