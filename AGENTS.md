...
Both scanners give Workers stable invocation-local IDs (`lane-name:worker-number`) and include the Worker plus song ID/name in page-request `start`/`success`/`failure` activity. Source `JobManager` groups its Worker map by song; single-song `ParallelJobManager` tracks only an O(1) Worker-ID set and publishes that task song with the set size. Both clear activity on every terminal path. Source's legacy-compatible `currentSong`/`onSongProgress` remains aggregate progress only; live activity comes from `activeSongs`.
...
- Source and parallel live activity comes from request activity keyed by Worker. `activeSongs` describes only requests currently in flight and groups multiple Workers on the same song; it is not a completion list, queue snapshot, or promise that a missing song is finished. Source `currentSong` remains an aggregate compatibility field, not the live-activity data source. Page numbering shown to users is one-based.
...
- `hostConcurrency` is the per-task proxy transport cap, accepted by source/parallel start and estimate APIs as `1..32` with default 8. `ProxyTransportGate` enforces that many in-flight first-hop requests across all task lanes and serializes subsequent starts at 80 ms plus a uniform integer jitter of 0..40 ms. Random samples are finite-checked and clamped before the jitter is capped. The gate runs inside each lane Governor callback, so retries reacquire it and delay/backoff waits do not hold capacity.
- Topology settings travel through the full runtime path: toolbar form -> source/parallel payload -> server validation -> lane selection/gate construction -> snapshot/log/resume -> toolbar and estimator. `workersPerLane` controls Worker-array cardinality; `hostConcurrency` controls the shared gate, not Worker creation. The renderer uses the same `ceil(hostConcurrency / workersPerLane)` rule for automatic lane display/estimate as the server selector.
- Managed-pool capacity and per-task exit selection are separate. `maxProxyLanes` accepts `0..32`: `0` targets `ceil(hostConcurrency / workersPerLane)` ordered verified exits; a positive value caps the task subset, always bounded by availability. Its selection metadata records requested versus actual lanes. Static-proxy/direct source scans still use one lane and do not shrink or rebuild the shared pool.
- `estimateCommentScan` models the per-lane cycle and the slower proxy-transport constraint: configured start delay plus jitter (0/half/full for optimistic/expected/conservative) or network latency divided by `hostConcurrency`. It reports configured Workers separately from `effectiveWorkers = min(totalWorkers, hostConcurrency)`.
...
- Scan traffic uses the task's configured `hostConcurrency` cap plus 80..120 ms randomized aggregate start spacing. Pool build/import/verify/refresh retains its separate 4-request/80-ms gate. These controls reduce burstiness but cannot hide the host IP from the upstream proxy provider or guarantee that a provider will never rate-limit the account; low latency is a health signal, not stealth.
...
- A lane is a verified proxy endpoint/egress; per-IP Workers share one Governor. Proxy-backed lanes share the configured `hostConcurrency` gate and randomized 80..120 ms aggregate start spacing. Ordinary failures requeue and back off; five consecutive final failures mark a lane unavailable, but task exhaustion waits for its active requests to settle and a late success revives it. This never deletes the lane from the managed pool. Managed/external pools are verified against both public egress IP and NetEase, isolated by IPv4 `/24` or IPv6 `/48`, configurable from the dashboard (current defaults 8 exits from 48 candidates), and rechecked in non-overlapping background rounds. `maxProxyLanes=0` selects enough ordered exits to cover `hostConcurrency` for the configured per-IP Worker count; a positive value caps the task subset without changing pool capacity.
...
For topology/metadata/performance changes, focused evidence must cover the pure auto/manual lane selector; indexed 500-ID metadata batching, partial first-batch retention plus durable checkpointing after a later failure, old-checkpoint hydration, and serial fallback; required `403/429` stickiness versus best-effort metadata `403` and `429` followed by successful required work; best-effort retry/budget/cancellation parity; and a pooled session where a cooled metadata lane is attempted once, skipped in later batches, healthy lanes continue, and the main scan remains usable. Preserve source Worker activity, source/parallel checkpoint callback counters, FIFO queue behavior, both scanners' clustered-failure/late-success race, `recordSuccess()` wakeup, the shared listener deadline, and atomic newer-schema rejection without rollback. Keep the server integration regression proving a failed source start does not publish its candidate result path; asset regex tests prove wiring only. In a real browser, verify sticky/fixed navigation, repeat-click close behavior, every output tab, focus/ARIA/reduced motion, job turnover, and SSE-during-snapshot order; historical comment timestamps must disagree with arrival order so an accidental timestamp sort is detectable.
...
- Per-IP Governors and the task-wide proxy gate protect different boundaries. Do not remove either, place the gate outside Governor delay/backoff, or count configured Workers above `hostConcurrency` as guaranteed simultaneous proxy capacity.
...
- The top task bar owns target/mode, `hostConcurrency` (1..32), task exit upper bound (0 automatic), topology text, and start/stop. Topology copy distinguishes selected/available exits, host-wide request capacity, and per-exit Worker count; never present configured Workers as guaranteed simultaneous requests.
- The center owns total progress and compact output tabs. `activity` is a full-width table of current status, song name/ID, and active Workers sourced from real source/parallel `activeSongs`; the right inspector is only for the proxy pool and nodes. Neither activity UI nor result rows determine durable completion.
...
- On desktop the proxy inspector is sticky beneath the header; at `<=1120px` it becomes a collapsible fixed overlay. The toolbar context remains visible and wraps through the 900..1120 px range. At `<=820px`, the sticky rail becomes a six-item top row while drawer/inspector stay viewport overlays. Current cache-busters are `styles.css?v=28` and `app.js?v=31`; bump the affected asset token on every change.
- User-triggered drawer/mode/pool transitions use short transform/opacity motion and honor `prefers-reduced-motion`; never animate dense live lists or polling decoration.
